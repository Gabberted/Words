...
def ProcessWord(_word,word):
	...
	try:
		...
		boKnown=boWordKnown(word,_word)
		debugprint("====================================")
		debugprint("Do we know the word : " + str(boKnown))
		if int(boKnown)==0:
			#not found so lets add it to word
			debugprint("No, so lets add it: " + str(boKnown))
			addNewWordToWord(word,_word)
		else:
			#word=args.recursive
			debugprint("Yes, so lets update it: " + str(boKnown))
			addKnownWordToWord(_word,word)
...
def PrintSum():
	...
	strQ="Select word from word where indx='" + strIndx  + "'"
	#strName=ExecuteQuery(strQ)
...
def queryURL(strQUrl,strSUrl):
	...
	debugprint("Requesting url: " + strQUrl)
...
def createTemplate():
	try:
		debugprint("Entering")
		initWord("Template")
		strList=queryURL(strQUrl,strSUrl)
		debugprintlist(strList)
		debugprint("Fetched list. Now initializing ProcessWordGenesis function")
		for word in strList:
			ProcessWordGenesis("Template",word)
		debugprint("Leaving")
	except:
		raise
		debugprint("Error")
...
if args.genesis_sead:
	...
	ProcessWordGenesis("template", strWord)
	print("Now recursive probing")
	#doOwnLoop(strWord)
if args.show_top_words:
	print("Show top words still has to be implemented")
	strQ="Select * from word order by count DESC limit " + args.show_top_words
	cur=ReturnCursor(strQ)
	print("-----------")
	for row in cur:
		iRow=0
...
if args.recursive:
	...
	strArg=args
	debugprint("URL Q: " + strQUrl)
	debugprint("URL S: " + strSUrl)
...
if args.process_db_tables:
	...
	for row in cur:
		strRow=row[0]
		strQUrl="https://duckduckgo.com/?q=" + strRow
		str_ListWords=queryURL(strQUrl,strSUrl)
		for strRow2 in str_ListWords:
			if str(strRow2)!="None":
				#initWord(strRow)
				ProcessWord(strRow2,strRow)
	#print("-----------")
